...
const HYPERVELOCE_SPEED = 60;

export class HyperVeloceNukeExecution implements Execution {
  private active = true;
  private mg: Game;
  ...
  tick(ticks: number): void {
    if (this.nuke === null) {
      ...
      if (spawn === false) {
        console.warn(`cannot build HyperVeloce nuke`);
        this.active = false;
        return;
      }
      this.src = spawn;
    ...
    if (!this.nuke.isActive()) {
      console.log(`HyperVeloce nuke destroyed before reaching target`);
      this.active = false;
      return;
    }
  ...
  private detonate() {
    ...
    for (const unit of this.mg.units()) {
      if (
        ...
        unit.type() !== UnitType.HyperVeloceBomb &&
